...
//   }
  ...
  std::int16_t mesh_dim = geometry.dim();
   ...
   const std::shared_ptr<const dolfinx::common::IndexMap> c_map = topology->index_map(c_dim);
   ...
   values.reserve(num_entities);

   for (int i = 0; i < (int)num_entities; ++i)
        values.push_back((double)(i+c_offset));

   auto meshtags = std::make_shared<mesh::MeshTags<U>>(
            mesh::create_meshtags<U>(topology, c_dim,
                                *entities,
                                values));
